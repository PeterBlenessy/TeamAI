<template>
    ...
    <q-toolbar class="col-6">
        <q-chip v-if="!isCreateImageSelected" icon="model_training" :label="model" size="sm" clickable disabled
            @click="showQuickSettings = (showQuickSettings != 'model') ? 'model' : ''">
        ...
        <q-chip v-if="!isCreateImageSelected" v-for="persona in personas" :key="persona.id" size="sm" clickable removable
    ...
    <q-toolbar class="col-6">
        ...
        <q-slider v-if="showQuickSettings == 'choices'" :model-value="choices" @change="val => { choices = val }" snap :min="1" :max="10" :step="1" :markers="1"
            label />
